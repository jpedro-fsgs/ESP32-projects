...
void mostrarPergunta(const char *pergunta)
{

    u8g2.clearBuffer();
    drawWrappedText(u8g2, 0, 12, 128, pergunta);
    u8g2.sendBuffer();
}
...
void correto()
{
    ...
    perguntasCertas++;
    u8g2.clearBuffer();
    u8g2.drawStr(0, 32, "CORRECT");
    std::string resultado = std::to_string(perguntasCertas) + "/" + std::to_string(perguntasFeitas);
    u8g2.drawStr(0, 48, resultado.c_str());
    u8g2.sendBuffer();
}
